...
const { goodOrService, dialogVisible ,markDialogVisible , reason , markVal , markOption, currentAction , markAsUpdatedArr } = useState({
  ...
  currentAction:'reject' as 'reject' | 'return',
  markAsUpdatedArr:{}
})
...
function markAsToBeUpdated(param)  {
  if(!(goodOrService.value.status ==='amended' || goodOrService.value.status === 'Approved')) return

  const {rowIndex , columnIndex } = param
  if(!markAsUpdatedArr.value[rowIndex]) markAsUpdatedArr.value[rowIndex] = {}
  if(markAsUpdatedArr.value[rowIndex] && markAsUpdatedArr.value[rowIndex][columnIndex]){
    markAsUpdatedArr.value[rowIndex][columnIndex] = 0
  }else{
    markAsUpdatedArr.value[rowIndex][columnIndex] = 1
  }
  console.log(markAsUpdatedArr.value);
...
function confirm(){
  currentAction.value ==='return' ? Approver.handleReturn(goodOrService.value,reason): Approver.handleReject(goodOrService.value,reason,'fo')
  reason.value = ''
  dialogVisible.value = false
  router.push('/applicationForFO')
}
...
<template>
  <div ref="container" class="container">
    ...
    style=";z-index: 1000;position: fixed;top: 80px;transform: translateX(0px)">
      <div class="box-card" style="height: 60px;padding: 20px 20px;padding-bottom: 0px;background-color:#FFF;box-shadow: 0 5px 10px #CDCDCD;">
        <div style="display: flex;justify-content: space-between;align-items: center;">
          ...
          <div v-if="goodOrService.status ==='amended' ||  goodOrService.status ==='Approved'" style="display: flex;">
            <el-button type="primary"  :disabled="Approver.canSubmit('approve',paymentTableData)" @click="Approver.handleApprove(goodOrService,'fo');router.replace('/applicationForFO')" >Approve </el-button>
            <el-button type="danger"   :disabled="Approver.canSubmit('reject',paymentTableData)" @click="()=>{dialogVisible = true ; currentAction = 'reject'}" >Reject </el-button>
            <el-button type="success"  :disabled="Approver.canSubmit('return',paymentTableData)" @click="()=>{dialogVisible = true ; currentAction = 'return'}" >Return</el-button>
          </div>
        </div>
      </div>
    </div>
    <div v-if="basicForm" class="app-container">
      ...
      <div ref="paymentContainer">
        <el-card   shadow="never" class="payment" style="margin-bottom: 20px;">
          ...
          <vxe-table
              ...
              @cell-dblclick="markAsToBeUpdated"
            >
              ...
              <vxe-column  title="Operations" width="200" >
                <template #default="{ row, rowIndex }">
                  <div>
                    <el-button :disabled="Approver.canUseMark(goodOrService,'fo')" :size="'small'" @click="Approver.handleMark(rowIndex,true,reason,paymentTableData,goodOrService)" :type="'primary'">Approve</el-button>
                    <el-popover :visible="row.showPopover" placement="top" :width="350" :trigger="'click'">
                      ...
                      <template #reference>
                        <el-button :disabled="Approver.canUseMark(goodOrService,'fo')" :size="'small'" :type="'danger'" @click="row.showPopover = true" >Reject</el-button>
                      </template>
                    </el-popover>
                  </div>
                </template>
      ...
      <!-- <div  ref="paymentRefs" v-for="(item,index) in paymentMethodCached"  >
    ...
    <SideNavigationBar review @click="onClick" :dataList="dataList" :currentTop="currentTop"/>
